...
import { ArrowLeft, Lightbulb, Calendar, Lock, Mail } from 'lucide-react';
...
import { useCreateCapsule, useBackendHealth } from '../hooks/useQueries';
import { saveReminder, requestNotificationPermission, canShowNotifications, calculateReminderTimes } from '../lib/reminders';
...
export function CreateCapsulePage({ initialType, onNavigate }: CreateCapsulePageProps) {
  const [step, setStep] = useState<'type' | 'form'>(initialType ? 'form' : 'type');
  const [capsuleType, setCapsuleType] = useState<'love' | 'prediction' | null>(initialType || null);
  
  // Love capsule fields
  const [loveCategory, setLoveCategory] = useState<LoveCategory>(LoveCategory.appreciation);
  const [message, setMessage] = useState('');
  const [note, setNote] = useState('');
  
  // Prediction capsule fields
  const [predictionCategory, setPredictionCategory] = useState<PredictionCategory>(PredictionCategory.personal);
  ...
  const { data: isBackendHealthy, isLoading: isHealthCheckLoading, refetch: refetchHealth } = useBackendHealth();
  ...
  const handleSubmit = async () => {
    ...
    if (!unlockDate) {
      ...
      addDebugLog(`Validation result: ${validationResult} - ${validationReason}`);
    ...
    try {
      ...
      const capsule = await createCapsuleMutation.mutateAsync({
        ...
        loveData: capsuleType === 'love' ? {
          category: loveCategory,
          message,
          note: note.trim() || undefined,
          anonymous: false
        } : undefined,
        predictionData: capsuleType === 'prediction' ? {
      ...
      if (enableReminder) {
        ...
        addDebugLog(`  - 24h reminder: ${reminderTimes.twentyFourHour.toLocaleString()}`);
        addDebugLog(`  - 1h reminder: ${reminderTimes.oneHour.toLocaleString()}`);
        addDebugLog(`  - Unlock time: ${reminderTimes.unlock.toLocaleString()}`);
        ...
        if (canShowNotifications()) {
          try {
            ...
            toast.success('Reminder set! You\'ll be notified 24h and 1h before unlock.');
          } catch (error) {
      ...
      if (errorMessage.includes('storage not confirmed')) {
        toast.error('Error saving capsule. Please try again later — storage not confirmed.');
  ...
  const isLockButtonDisabled = createCapsuleMutation.isPending || !isBackendHealthy || isHealthCheckLoading;
  ...
  if (step === 'type') {
    return (
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        ...
        <BackendHealthBanner isHealthy={isBackendHealthy ?? true} isLoading={isHealthCheckLoading} />

        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold mb-4">Choose Capsule Type</h1>
          <p className="text-muted-foreground">Select the type of time capsule you want to create</p>
        </div>
      ...
      <Card>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-2xl">
                {capsuleType === 'love' ? 'Create Message Capsule' : 'Create Prediction Capsule'}
              </CardTitle>
              <CardDescription>
                {capsuleType === 'love' 
        ...
        <CardContent className="space-y-6">
          {capsuleType === 'love' ? (
            <>
              <div className="space-y-2">
                ...
                <Select value={loveCategory} onValueChange={(v) => setLoveCategory(v as LoveCategory)}>
                  <SelectTrigger id="love-category">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value={LoveCategory.appreciation}>Appreciation</SelectItem>
                    <SelectItem value={LoveCategory.encouragement}>Encouragement</SelectItem>
                    <SelectItem value={LoveCategory.celebration}>Love</SelectItem>
                    <SelectItem value={LoveCategory.apology}>Apology</SelectItem>
                    <SelectItem value={LoveCategory.reflection}>Closure</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="note">Short Note (Optional, max 120 characters)</Label>
                <Input
                  id="note"
                  ...
                  maxLength={120}
                />
                <p className="text-xs text-muted-foreground">{note.length}/120 characters</p>
              </div>
            </>
          ) : (
            <>
              <div className="space-y-2">
                ...
                <Select value={predictionCategory} onValueChange={(v) => setPredictionCategory(v as PredictionCategory)}>
                  <SelectTrigger id="pred-category">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value={PredictionCategory.personal}>Personal</SelectItem>
                    <SelectItem value={PredictionCategory.work}>Work</SelectItem>
                    <SelectItem value={PredictionCategory.money}>Money</SelectItem>
                    <SelectItem value={PredictionCategory.sportsEvents}>Sports/Events</SelectItem>
                    <SelectItem value={PredictionCategory.life}>Life</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="confidence">Confidence Level (Optional, 0-100%)</Label>
                <Input
                  id="confidence"
                  type="number"
                  min="0"
              ...
              <div className="space-y-2">
                <Label htmlFor="explanation">Explanation (Optional, max 120 characters)</Label>
                <Input
                  id="explanation"
                  ...
                  maxLength={120}
                />
                <p className="text-xs text-muted-foreground">{explanation.length}/120 characters</p>
              </div>
            </>
          )}
          ...
          <div className="bg-muted/50 rounded-lg p-4 space-y-3">
            ...
            {enableReminder && (
              <p className="text-xs text-muted-foreground">
                You'll receive browser notifications before this capsule unlocks. 
          ...
          {debugLogs.length > 0 && (
            <div className="mt-6 border-2 border-primary/30 rounded-lg p-4 bg-muted/50">
              <h3 className="text-sm font-semibold mb-2 text-primary">Debug Log</h3>
